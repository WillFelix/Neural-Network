...
public class JogoGenetico {
	...
	public void buscaSolucao() {
		...
		if (!solucao) {
			System.out.println("Solucao com custo ZERO nao encontrada! A solucao com o menor custo eh:");
			System.out.println("populacao Atual: ");
			this.imprimirPopulacao(this.populacao);
		} else {
	...
	// M�todos de conversao inteiro para bit e bit para inteiro
	...
	public void mutacao(Tabuleiro tabFilho) {
		Random r = new Random();
		int bitRandom, mutRandom;
		int[] bits, tabuleiro = tabFilho.getTabuleiro();
	...
	/**
	 * M�todo que imprime a configuracao final onde nenhuma rainha se ataca
	 */
	...
	public void imprimirPopulacao(ArrayList<Tabuleiro> populacao) {
		...
		for (Tabuleiro tab : populacao) {
			...
			//this.imprimirPopulacao(populacao);
			this.imprimirTab(tab);
		}
	}
	 ...
	 * @param ind
	 */
	private void imprimirTab(Tabuleiro tab){
		int[] tabAux = tab.getTabuleiro();
		
		System.out.println("Custo:" +tab.getCusto());
		System.out.println("Sofreu Mutacao: " + tab.isMutacao());
		System.out.print("[");
		for(int i = 0;i < 8; i++){
